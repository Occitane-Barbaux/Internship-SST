...
print(Y_ref)
if Type_station =="Oui":
    path_Y_Obs_Brut=path_Y_Obs_Brut    +st_n
    bashCommand = "cdo -selvar,TX -yearmax "+path_Y_Obs_Brut+"_*.nc"+" "+pathTemp+"/Yo_test.nc"
    sub.call(bashCommand , shell=True,stdout=sub.DEVNULL )
    ...
    path_Y_Obs_Brut=path_Y_Obs_Brut    +"*_"+Y_ref+".txt"
    dYo =  pd.read_csv(glob.glob(path_Y_Obs_Brut)[0],sep="\t")
    dates=dYo['date'].values
    Yo  = pd.DataFrame( dYo['TX_MA'].values.squeeze() , columns = ["Yo"] , index = [int(i[:4]) for i in dates] )
    Yo=Yo[np.isfinite(Yo.values)]
# kelvin to celsius
